...
1. (Opcional) Puede ocultar ciertos campos de la interfaz de usuario del esquema. Estos campos son campos de creación de informes calculados por el servidor que Adobe rellena en el servidor: su SDK o API no los envía y no afectan a la recopilación de datos. Ocultarlos no tiene ningún impacto funcional; solo reduce el ruido visual al examinar el esquema en la interfaz de usuario de AEP. Estos campos solo hacen referencia a los del grupo de campos `MediaAnalytics Interaction Details`.
...
1. Cree una nueva secuencia de datos como se describe en [Configurar una secuencia de datos](https://experienceleague.adobe.com/docs/experience-platform/edge/datastreams/overview.html?lang=es).
   ...
   * En el campo **[!UICONTROL Esquema de evento]**, seleccione el esquema que creó en [Configurar el esquema en Adobe Experience Platform](#set-up-the-schema-in-adobe-experience-platform).

     >[!IMPORTANT]
     >
     >Seleccione **[!UICONTROL Guardar]**; no seleccione **[!UICONTROL Guardar y agregar asignación]**. Si se selecciona **[!UICONTROL Guardar y agregar asignación]**, se producirán errores de asignación en el campo Marca de tiempo.
