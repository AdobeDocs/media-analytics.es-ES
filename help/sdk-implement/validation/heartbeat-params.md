...
| l:event:duration | Media SDK | (Required)<br/><br/>This value is set internally (in milliseconds) by the Media SDK, not by the player. Se utiliza para calcular las métricas de tiempo invertido en el servidor. P. ej.: a. media. totaltimeplayed se calcula como una suma de la duración de todos los latidos Play (type = play) generados. <br/>*Nota:* Este parámetro se establece en 0 para determinados eventos porque son "eventos de cambio de estado" (por ejemplo: type = complete, type = chapter_ complete o type = bitrate_ change). |
...
| l: recurso: duration/l: recurso: length <br/>(cambio de nombre de duración) | Videoinfo | (Required)<br/><br/>The video asset length of the main asset. |
...
| s:sc:rsid | ID (o identificadores) del grupo de informes | (Requerido)<br/><br/>RSID de Adobe Analytics donde se deben enviar los informes. |
| s:sc:tracking_server | MediaHeartbeatConfig | (Requerido)<br/><br/>Servidor de seguimiento de Adobe Analytics. |
...
| s:sp:hb_version | Media SDK | (Requerido)<br/><br/>El número de versión de la biblioteca del SDK de medios que efectúa la llamada. |
...
| s:asset:resolver | Adbreakinfo | (Requerido)<br/><br/>El resolver publicidad. |
...
| s:meta:custom_chapter_metadata.x | Chapterinfo | (Opcional)<br/><br/>Metadatos del capítulo personalizado. |

## Evento Fin de la sesión

| Nombre  | Fuente de datos | Descripción   |
| ---  | --- | --- |
| s:event:type=end | Media SDK | (Requerido)<br/><br/> La variable `end``close` |
