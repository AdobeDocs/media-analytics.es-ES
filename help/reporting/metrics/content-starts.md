...
El servidor multimedia establece `mediaReporting.sessionDetails.isPlayed = true` la primera vez que se recibe un evento `media.play` para el contenido principal. La métrica se activa en ese evento de reproducción, pero se comunica en la llamada de cierre. Para calcular la tasa de colocación previa a la emisión, use `(Media starts − Content starts) / Media starts`.
