...
El servidor multimedia establece `mediaReporting.sessionDetails.isCompleted = true` cuando se recibe un evento `media.sessionComplete`. La métrica se recoge en la llamada de cierre. Una sesión que agota el tiempo de espera sin un `sessionComplete` explícito no se cuenta como una finalización.
