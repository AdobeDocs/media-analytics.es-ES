...
| ---  | --- | --- |
| s:event:tipo | Media SDK | (Obligatorio)<br/><br/>Tipo de evento que se rastrea. Tipos de eventos: <ul> <li> s:event:tipo=inicio </li> <li> s:event:tipo=completo </li> <li> s:event:tipo=inicio_capítulo </li> <li> s:event:tipo=capítulo_completo </li> <li> s:event:tipo=búfer </li> <li> s:event:tipo=pausa </li> <li> s:event:tipo=reanudar </li> <li> s:event:tipo=cambio_bitrate </li> <li> s:event:tipo=aa_inicio </li> <li> s:event:tipo=stall </li> <li> s:event:tipo=fin </li> </ul> |
...
| l:asset:duración / l:asset:longitud <br/>(Renombrado de longitud duración) | VideoInfo | (Obligatorio)<br/><br/>La duración del recurso de vídeo del recurso principal. |
| s:asset:editor | MediaHeartbeatConfig | (Obligatorio)<br/><br/>El editor del recurso. |
| s:asset:vídeo_id | VideoInfo | (Obligatorio)<br/><br/>ID que identifica de forma exclusiva el vídeo en el catálogo del editor. |
| s:asset:tipo | Media SDK | (Obligatorio)<br/><br/>El tipo de recurso (principal o anuncio). |
| s:stream:tipo | VideoInfo | (Obligatorio)<br/><br/>El tipo de flujo. Puede ser uno de los siguientes: <ul> <li> live </li> <li> vod </li> <li> lineal </li> </ul> |
...
| s:cuser:id_usuario_cliente_x | MediaHeartbeatConfig | (Obligatorio)<br/><br/>Todos los ID de usuario de cliente definidos en Audience Manager. |
...
| s:sc:servidor_seguimiento | MediaHeartbeatConfig | (Obligatorio)<br/><br/>Servidor de seguimiento de Adobe Analytics. |
...
| s:sp:nombre_jugador | VideoInfo | (Obligatorio)<br/><br/>El nombre del reproductor (el software del reproductor real, utilizado para identificar al mismo). |
| s:sp:canal | MediaHeartbeatConfig | (Opcional)<br/><br/>Canal donde el usuario ve el contenido. Para una aplicación móvil, el nombre de la aplicación. Para un sitio web, el nombre de dominio. |
| s:sp:hb_versión | Media SDK | (Obligatorio)<br/><br/>El número de versión de la biblioteca de Media SDK que emite la llamada. |
...
| Nombre | Fuente de datos | Descripción   |
| ---  | --- | --- |
| s:event:fuente | Media SDK | (Obligatorio)<br/><br/>Origen del error, ya sea que esté en el reproductor o en la aplicación. |
...
| s:asset:pod_posición | AdBreakInfo | (Obligatorio)<br/><br/>Índice del anuncio dentro de la secuencia (el primer anuncio tiene el índice 0, el segundo tiene el índice 1, etc.). |
| s:asset:resolver | AdBreakInfo | (Obligatorio)<br/><br/>La resolución del anuncio. |
| s:meta:metadatos_anuncio_personalizados.x | MediaHeartbeat | (Opcional)<br/><br/>Los metadatos de anuncios personalizados. |

## Eventos de capítulo

| Nombre | Fuente de datos | Descripción   |
| ---  | --- | --- |
| s:stream:capítulo_sid | Media SDK | (Obligatorio)<br/><br/>Identificador único asociado a la instancia de reproducción del capítulo.  <br/> **Nota:** Un capítulo se puede reproducir varias veces debido a las operaciones seek-back realizadas por el usuario. |
| s:stream:nombre_capítulo | ChapterInfo | (Opcional)<br/><br/>El nombre descriptivo del capítulo (por ejemplo, que sea comprensible). |
| s:stream:id_capítulo | Media SDK | (Obligatorio)<br/><br/>ID único del capítulo. Este valor se calcula automáticamente en función de la siguiente fórmula: <br/>`MD5(video_id) +`<br/>` "_" +`<br/>`chapter_pos` |
| l:stream:pos_capítulo | ChapterInfo | (Obligatorio)<br/><br/>Índice del capítulo en la lista de capítulos (a partir del 1). |
| l:stream:desplazamiento_capítulo | ChapterInfo | (Obligatorio)<br/><br/>El desplazamiento del capítulo (expresado en segundos) dentro del contenido principal, sin incluir los anuncios. |
| l:stream:longitud_capítulo | ChapterInfo | (Obligatorio)<br/><br/>La duración del capítulo (expresada en segundos). |
| s:meta:metadatos_capítulo_personalizados.x | ChapterInfo | (Opcional)<br/><br/>Metadatos de capítulo personalizados. |

## Evento Fin de la sesión

| Nombre | Fuente de datos | Descripción   |
| ---  | --- | --- |
| s:event:tipo=fin | Media SDK | (Obligatorio)<br/><br/> El `end` `close` |
